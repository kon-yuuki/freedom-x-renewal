...
class GenericAccordion {
  ...
  init() {
    ...
    this.accordionElements.forEach((accordionEl, index) => {
      ...
      accordionEl.addEventListener('click', (e) => {
        ...
        // アコーディオンのボタン以外のa, button要素がクリックされた場合はスキップ
        const clickedElement = e.target.closest('a, button');
        if (clickedElement && !clickedElement.closest(this.options.buttonSelector)) {
          this.log('GenericAccordion: アコーディオン以外のaまたはbutton要素がクリックされたため、処理をスキップします');
          return;
        }
  ...
  show(el) {
    ...
    this.log(`GenericAccordion: 要素の高さを ${height}px に設定してアニメーション開始`);
    el.style.height = `${height}px`;
    el.setAttribute('aria-hidden', 'false');

    this.log('GenericAccordion: トランジション終了イベントリスナーを追加');
    el.addEventListener('transitionend', handleTransitionEnd);
    
    // 追加: トランジションが発火しない場合のフォールバック
    this.log('GenericAccordion: トランジションタイムアウトを設定 (フォールバック)');
    setTimeout(() => {
      this.log(`GenericAccordion: トランジションタイムアウト発生 (${this.options.animationDuration + 50}ms後)`);
      
      if (this.isAnimating) {
        this.log('GenericAccordion: アニメーションがまだ完了していないため、フォールバック処理を実行');
        el.style.height = 'auto';
        el.style.overflow = '';
  ...
  hide(el, closeNested = false) {
    this.log('GenericAccordion: hide() メソッドが呼び出されました');
    this.log(`GenericAccordion: 対象要素: ${el}, ネストを閉じる: ${closeNested}`);
    ...
    if (closeNested) {
      ...
      this.log(`GenericAccordion: ${nestedOpenAccordions.length}個の開いているネストされたアコーディオンを検出`);
      
      nestedOpenAccordions.forEach((nestedAccordion, index) => {
        this.log(`GenericAccordion: ネストされたアコーディオン[${index}]を閉じます`);
        const nestedContent = nestedAccordion.querySelector(this.options.nestedContentSelector);
        
        if (nestedContent) {
          this.log(`GenericAccordion: ネストされたアコーディオン[${index}]のコンテンツを非表示にします`);
          this.hide(nestedContent);
          nestedAccordion.classList.remove(this.options.openClass);
        } else {
          this.warn(`GenericAccordion: ネストされたアコーディオン[${index}]のコンテンツ要素が見つかりません`);
        }
      });
    }
 
    this.log(`GenericAccordion: 現在の要素の高さ: ${el.offsetHeight}px`);
    el.style.height = `${el.offsetHeight}px`;
    el.offsetHeight; // 強制的なリフロー
    this.log('GenericAccordion: 強制リフロー完了');
    
    this.log('GenericAccordion: トランジションプロパティを設定');
    el.style.overflow = 'hidden';
    el.style.transition = `${this.options.animationDuration}ms ${this.options.easing}`;
    el.style.transitionProperty = this.options.transitionProperty;
    this.log(`GenericAccordion: トランジション設定 - duration: ${this.options.animationDuration}ms, easing: ${this.options.easing}, properties: ${this.options.transitionProperty}`);
    
    this.log('GenericAccordion: 要素の高さを0に設定してアニメーション開始');
    el.style.height = '0';
