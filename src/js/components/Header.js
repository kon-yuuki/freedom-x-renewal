...
class Header {
  ...
  fixControl() {
    ...
    const logo = document.querySelector('.l-header__logo--img use');
    const bgBlue = document.body.classList.contains('blue');//bodyがblueクラスを持っているかどうか
    window.addEventListener("scroll", () => {
      if (scrollY > this.headerHeight + adjustNum) {
        this.header.classList.add('is-fixed')
  
        if (bgBlue) {
          logo.setAttribute('href', '#i-logo');
        }
      } 
      else {
        ...
        if (bgBlue) {
          logo.setAttribute('href', '#i-logo-white');
        }
      }
    })
  }
